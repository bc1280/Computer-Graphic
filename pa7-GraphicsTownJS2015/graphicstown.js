...
window.onload = function() {
    ...
    var sliders = makeSliders([["Time",0,24,12]]);
    var TOD = document.createElement("p");
    controls.appendChild(TOD);
    ...
    function draw() {
        ...
        var tod = Number(sliders.Time.value);
        TOD.innerText = "Time: " + sliders.Time.value + ":00 (24-hour clock)";
        var sunAngle = Math.PI * (tod - 6) / 12;
        if (sunAngle <= Math.PI && sunAngle >= 0) {
            var sunDirection = [Math.cos(sunAngle), Math.sin(sunAngle), 0];
        } else {
            var sunDirection = [0, 0, 0];
        }
